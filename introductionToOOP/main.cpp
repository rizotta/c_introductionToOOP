...
public:
	...
	//{
	//	x = y = 0; // инициализаци€ значений по умолчанию, то, что нужно в конструкторе
	...
	~Point() // деструктор, обычно пишетс€ внизу, после конструкторов
	{
	...
	double distance(Point new_point)const // возвращает рассто€ние до указанной точки
	{
		double x_dist;
		double y_dist;
...
// }A, B, C; - объ€вление объектов сразу
...
void main()
{
	setlocale(LC_ALL, "");
#ifdef STRUCT_POINT
	// type name - объ€вление как объ€вление переменной
	int a;	// ќбъ€вл€ем переменную 'a' типа 'int'

	// ќбъ€вл€ем переменную 'A' типа 'Point', 
	// ещЄ говор€т —оздаЄм объект (экземпл€р) структуры 'Point'
	Point A;
	A.x = 2;
	A.y = 3;
	cout << A.x << tab << A.y << endl;

	Point* pA = &A; // —оздаЄм указатель на объект нашей структуры
	cout << pA->x << tab << pA->y << endl;

#endif // STRUCT_POINT
	
	//double x, y;
	//cout << "¬ведите координаты точки: "; cin >> x >> y;
	//A.set_x(2);
	//A.set_y(4);
